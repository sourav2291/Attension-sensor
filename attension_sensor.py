...
# ---------------- MAIN FUNCTION ----------------
def run_attension_sensor(logged_user="User"):
    # ---------------- MediaPipe Setup ----------------
    ...
    while True:
        ...
        # ---------- UI ----------
        panel = [
            (f"User : {logged_user}", (255, 255, 255)),
            ("Status : Distracted" if distraction_active else "Status : Focused",
             (0, 0, 255) if distraction_active else (0, 200, 0)),
            (f"Distractions : {distraction_count}", (0, 165, 255)),
            (f"Total Distracted : {int(total_distracted_time)} sec", (200, 200, 255))
        ]

        draw_info_panel(frame, panel)
    ...
    input("Press Enter to exit...")  # ✅ keeps EXE window open

# ---------------- RUN ----------------
if __name__ == "__main__":
    run_attension_sensor("TestUser")  # ✅ fixed spelling
